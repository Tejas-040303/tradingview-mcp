...
def find_setups(bars_by_tf, config=None):
    ...
    for sweep in sweeps:
        ...
        # ...but it has to be *soon* after. A sweep whose close predates every
        # bar we hold takes start=0 and would trigger against the first entry
        # bar in the window — a trade generated by where the data begins
        # rather than by the market. It matters at both edges: the sweep
        # timeframes are fetched with a lookback beyond the entry window, and
        # a walk-forward split hands the out-of-sample half a fresh start.
        late_by = times[start] - sweep['knowable_utc']
        if late_by > cfg['entry']['wait_bars'] * period(entry_tf):
            rejected.append({**_ref(sweep),
                             'reason': 'sweep is older than the entry window'})
            continue
