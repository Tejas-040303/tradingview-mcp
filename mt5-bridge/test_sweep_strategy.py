...
class TestClockDiscipline(unittest.TestCase):
    ...
    def test_a_sweep_older_than_the_entry_window_is_not_tradeable(self):
        # Otherwise it takes the first available bar and trades there — a
        # signal generated by where the data starts, not by the market.
        late = series('3', [(3993, 3997, 3992, 3996.5)] * 40,
                      start=START + 400 * ss.period('3'))
        out = ss.find_setups({'15': swept_low_series('15'), '3': late})
        self.assertEqual(out['setups'], [])
        self.assertTrue(any('older than the entry window' in r['reason']
                            for r in out['rejected']))

    def test_a_sweep_just_inside_the_window_is_still_tradeable(self):
        out = ss.find_setups({'15': swept_low_series('15'), '3': entry_series()})
